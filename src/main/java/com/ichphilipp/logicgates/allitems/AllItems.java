...
public class AllItems {
    private static final int MAX_STACK_SIZE_CHIPS = LogicGates.config.max_stack_size_chips.get();
    public static final DeferredRegister<Item> ITEMS =
            DeferredRegister.create(ForgeRegistries.ITEMS, LogicGates.MOD_ID);
    // -------------------------------------------------------------------------------------------------------
    public static final RegistryObject<Item> CHIP = ITEMS.register("chip",
            () -> new Item(new Item.Properties().tab(LogicGates.ITEM_GROUP)));
    // -------------------------------------------------------------------------------------------------------
    public static final RegistryObject<Item> NOT_GATE = ITEMS.register("not_gate",
            () -> new Chip(
                    new Item.Properties().tab(LogicGates.ITEM_GROUP).stacksTo(MAX_STACK_SIZE_CHIPS),
                    GateFrameTypes.NOT,
                    (L, B, R) -> !B,
                    new int[]{0,1,0}
            )
